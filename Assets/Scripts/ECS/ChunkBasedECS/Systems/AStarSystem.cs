...
public class AStarSystem : IInitSystem
{
    ...
    internal List<AStarNode> FindPath(AStarNode startTile, AStarNode goalTile, QuadTreeNodeData quadtreeNode,NativeArray<int2> directions, int mapWidth, int mapHeight)
    {
        ...
        while (openList.Length > 0)
        {
            ...
            GetNeighboursJob getNeighboursJob=new GetNeighboursJob()
            {
                  ...
                  MapWidth=mapWidth,
                  MapHeight=mapHeight,
                  TileChunks= _world.ChunkContainers[(ushort)(ComponentMask.CoordinateComponent | ComponentMask.TileComponent)],
                  QuadtreeChunk= _world.ChunkContainers[(ushort)(ComponentMask.QuadTreeLeafComponent)][0],
    ...
    private struct GetNeighboursJob : IJobParallelFor
    {
        ...
        [ReadOnly] public NativeList<Chunk> TileChunks;
        [NativeDisableUnsafePtrRestriction]
        [ReadOnly] public Chunk QuadtreeChunk;
